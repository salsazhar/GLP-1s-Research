"""
GLP-1 Demand vs Capacity Gap Analysis - Premium Visualization Suite

This module generates publication-ready figures highlighting the demand-capacity gap
across three scenarios (conservative, base, aggressive) with premium styling.
"""
...
# Alternative colorblind-friendly palette (commented for reference)
# COLORS = {
#     'conservative': '#1f77b4',  # Blue
#     'base': '#ff7f0e',          # Orange
#     'aggressive': '#2ca02c',    # Green
#     'demand': '#d62728',        # Red
#     'capacity': '#9467bd',      # Purple
#     'fill': '#ff7f0e',          # Orange for fills
...
def load_and_process_data(csv_path: str) -> Tuple[pd.DataFrame, Dict[str, pd.DataFrame]]:
    ...
    """
    # Read CSV with explicit dtype casting
    ...
    # Create long format for combined plots
    df_all = df.copy()
    
    # Create wide format for per-scenario plots
    dict_df = {}
    for scenario in df['scenario'].unique():
...
def create_figure_7_cover_gradient(output_dir: Path) -> None:
    ...
    # Create gradient from deep navy to gold
    for y in range(height):
        # Interpolate between navy and gold
        ratio = y / height
...
def main():
    ...
    # Figures 2-4: Individual scenario deep-dives
